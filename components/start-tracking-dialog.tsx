...
export function StartTrackingDialog({ open, onOpenChange }: StartTrackingDialogProps) {
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    try {
      const sessionId = startTracking(data.categoryId as CategoryId, data.metricId, data.notes)

      if (!sessionId) {
        toast({
          title: "Error starting tracking",
          description: "There was a problem starting the tracking session.",
          variant: "destructive",
        })
        return
      }
      ...
      toast({
        ...
        description: "Your activity tracking has been started successfully.",
      })
    } catch (error) {
