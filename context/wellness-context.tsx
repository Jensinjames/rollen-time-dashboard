...
interface WellnessContextType {
  ...
  filteredEntries: WellnessEntryData[]
  dateRange: DateRange
  ...
  addSubcategory: (categoryId: CategoryId, subcategory: WellnessSubcategory) => void
  updateSubcategory: (categoryId: CategoryId, subcategoryId: string, updates: Partial<WellnessSubcategory>) => void
  removeSubcategory: (categoryId: CategoryId, subcategoryId: string) => void
  ...
  filterCategories: (filter: string) => WellnessCategory[]
  sortCategories: (sortBy: "name" | "createdAt" | "lastUpdated") => WellnessCategory[]
}
...
export function WellnessProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadData = () => {
      try {
        ...
        // Load date range
        const savedDateRange = localStorage.getItem("wellnessDateRange")
        if (savedDateRange) {
          const parsedDateRange = JSON.parse(savedDateRange)
          setDateRange({
            from: new Date(parsedDateRange.from),
            to: new Date(parsedDateRange.to),
          })
        }
      } catch (error) {
  ...
  useEffect(() => {
    localStorage.setItem("wellnessDateRange", JSON.stringify(dateRange))
  }, [dateRange])

  // Category management functions
  const addCategory = (category: WellnessCategory) => {
    // Add createdAt and lastUpdated timestamps
    const newCategory = {
      ...category,
  ...
  const addSubcategory = (categoryId: CategoryId, subcategory: WellnessSubcategory) => {
    setCategories((prev) =>
      prev.map((cat) => {
  ...
  const updateSubcategory = (categoryId: CategoryId, subcategoryId: string, updates: Partial<WellnessSubcategory>) => {
    setCategories((prev) =>
      prev.map((cat) => {
  ...
  const removeSubcategory = (categoryId: CategoryId, subcategoryId: string) => {
    setCategories((prev) =>
      prev.map((cat) => {
        if (cat.id === categoryId && cat.subcategories) {
          return {
  ...
  const filterCategories = (filter: string) => {
    ...
    return categories.filter(
      (cat) =>
        ...
        cat.metrics.some((m) => m.name.toLowerCase().includes(lowerFilter)),
    )
  }

  // Sort categories based on criteria
  const sortCategories = (sortBy: "name" | "createdAt" | "lastUpdated") => {
